...
void init_direction_pins() {
	...
	PORTD->PCR[PTE20_PIN]&= ~PORT_PCR_MUX_MASK; 
	/*Set PORT E pin 20 to use ALT1 which is GPIO*/
	PORTD->PCR[PTE20_PIN]|= PORT_PCR_MUX(1);
	
	PORTD->PCR[PTE21_PIN]&= ~PORT_PCR_MUX_MASK; 
	/*Set PORT E pin 21 to use ALT1 which is GPIO*/
	PORTD->PCR[PTE21_PIN]|= PORT_PCR_MUX(1);
	
	PORTD->PCR[PTE22_PIN]&= ~PORT_PCR_MUX_MASK; 
	/*Set PORT E pin 22 to use ALT1 which is GPIO*/
	PORTD->PCR[PTE22_PIN]|= PORT_PCR_MUX(1);
	
	PORTD->PCR[PTE23_PIN]&= ~PORT_PCR_MUX_MASK; 
	/*Set PORT E pin 23 to use ALT1 which is GPIO*/
	PORTD->PCR[PTE23_PIN]|= PORT_PCR_MUX(1);
