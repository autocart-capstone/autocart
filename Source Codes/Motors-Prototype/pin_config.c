...
void init_PWM(void) {
	...
	PORTD->PCR[PTD0_PIN_BL]&= ~PORT_PCR_MUX_MASK; 
	/*Set PORT D pin 0 to use ALT4 which is TPM0_CH0*/
	PORTD->PCR[PTD0_PIN_BL]|= PORT_PCR_MUX(4);
	...
	PORTD->PCR[PTD2_PIN_FR]&= ~PORT_PCR_MUX_MASK; 
	/*Set PORT D pin 2 to use ALT4 which is TPM0_CH2*/
	PORTD->PCR[PTD2_PIN_FR]|= PORT_PCR_MUX(4);
	
	PORTD->PCR[PTD3_PIN_FL]&= ~PORT_PCR_MUX_MASK; 
	/*Set PORT D pin 3 to use ALT4 which is TPM0_CH3*/
	PORTD->PCR[PTD3_PIN_FL]|= PORT_PCR_MUX(4);
	
	PORTD->PCR[PTD4_PIN_BR]&= ~PORT_PCR_MUX_MASK; 
	/*Set PORT D pin 4 to use ALT4 which is TPM0_CH4*/
	PORTD->PCR[PTD4_PIN_BR]|= PORT_PCR_MUX(4);
...
void init_direction_pins(void) {
	...
	PORTE->PCR[PTE20_PIN_LF]&= ~PORT_PCR_MUX_MASK; 
	/*Set PORT E pin 20 to use ALT1 which is GPIO*/
	PORTE->PCR[PTE20_PIN_LF]|= PORT_PCR_MUX(1);
	
	PORTE->PCR[PTE21_PIN_LB]&= ~PORT_PCR_MUX_MASK; 
	/*Set PORT E pin 21 to use ALT1 which is GPIO*/
	PORTE->PCR[PTE21_PIN_LB]|= PORT_PCR_MUX(1);
	
	PORTE->PCR[PTE22_PIN_RF]&= ~PORT_PCR_MUX_MASK; 
	/*Set PORT E pin 22 to use ALT1 which is GPIO*/
	PORTE->PCR[PTE22_PIN_RF]|= PORT_PCR_MUX(1);
	
	PORTE->PCR[PTE23_PIN_RB]&= ~PORT_PCR_MUX_MASK; 
	/*Set PORT E pin 23 to use ALT1 which is GPIO*/
	PORTE->PCR[PTE23_PIN_RB]|= PORT_PCR_MUX(1);
	
	/*Set Pins to Output*/
	PTE->PDDR |= MASK(PTE20_PIN_LF)| MASK(PTE21_PIN_LB) | MASK(PTE22_PIN_RF)| MASK(PTE23_PIN_RB);
