...
def main_task():
    ...
    with serial.Serial("COM4", 115200) as ser:
        ...
        # Create figure, and initially clear it
        plt.ion()
        plt.figure(1)
        plt.clf()   # Prianna - Bren uncommented the plt.clf() back
       
        # Subplot for tracking where we are in the workspace
        # This plot is located at the top of the figure
        plt.subplot(212)
        plt.scatter(
            [A[0], B[0], C[0]],
            [A[1], B[1], C[1]],
        ...
        # Subplot for actually drawing the path in the workspace
        # This plot is located at the top of the figure
        plt.subplot(211)
        plt.scatter(
            [A[0], B[0], C[0]],
        ...
        # Initialize the list where the x and y coordinates of the drawing is saved
        new_drawing_idx = -1
        # x_pts = []
        # y_pts = []
        drawing = []
        ...
        while True:
            ...
            plt.subplot(212)
            plt.cla()       # Clear axis then scatter re-plot the base stations
            plt.scatter(
                [A[0], B[0], C[0]],
                [A[1], B[1], C[1]],
                label="base stations",
            ...
            if separate_draw:       # If separate draw, clear the scatter plot previouly shown
                plt.subplot(211)
                plt.cla()            # Clear axis then scatter re-plot the base stations
                plt.scatter(
                    [A[0], B[0], C[0]],
            ...
            for name, position in positions.items():
                plt.subplot(212)        # Subplot for tracking where we are in the workspace
                plt.scatter(position[0], position[1], label=name)
                plt.subplot(211)        # Subplot for actually drawing the path in the workspace
                plt.scatter(position[0], position[1], label=name)

            plt.gca().set_aspect("equal")
            plt.legend(loc="center left", bbox_to_anchor=(1.0, 0.5))

            # Plot base stations -> Bren: I commented this out because I moved it a few lines up with a condition
            # plt.subplot(211)
            # plt.scatter(
            #     [A[0], B[0], C[0]],
            #     [A[1], B[1], C[1]],
            ...
            if draw_button_on == 1:
                ...
                new_drawing_idx += 1
                drawing.append(([],[])) # Create empty list of x and y coordinates
                
                # Only append to the list of guessed positions if is it not None
                if guessed_position is not None:
                    drawing[new_drawing_idx][0].append(guessed_position[0]) # Add x coord in the drawing list
                    drawing[new_drawing_idx][1].append(guessed_position[1]) # Add y coord in the drawing list
                    # guessed_positions.append(guessed_position)
                # print(guessed_positions)

                # Extract x, y coordinate from list
                x_elem = drawing[new_drawing_idx][0]
                y_elem = drawing[new_drawing_idx][1]
